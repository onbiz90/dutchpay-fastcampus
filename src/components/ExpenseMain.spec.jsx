...
const renderComponent = () => {
    ...
    // Error Message - desc, amount, payer
...
describe("비용 정산 메인 페이지", () => {
    ...
    describe("비용 추가 컴포넌트", () => {
        ...
        test("비용 추가에 필수적인 값들을 입력한 후 추가 버튼 클릭시, 저장에 성공", async () => {
            const { descInput, amountInput, payerInput, addButton, 
                descErrorMessage, amountErrorMessage, payerErrorMessage } = renderComponent()
    ...
    describe("새로운 비용이 입력 되었을 때,", () => {
        ...
        test("날짜, 내용, 결제자, 금액이 데이터가 정산 리스트에 추가된다", () => {
            const expenseListComponent = screen.getByTestId("expenseList")
        ...
        describe("공유 버튼 클릭시", () => {
            describe("모바일에서", () => {
                beforeEach(() => {
                    ...
                    Object.defineProperties(window.navigator, "userAgent", { value: "iPhone" })
                })
